<?php
    ...
    $sql="select * from users where username like '%$query%' AND id!='$users_id'";
    ...
    $hasChats=false;
        if(mysqli_num_rows($res)>0)
        {
            ...
            while($row=mysqli_fetch_assoc($res))
                {
                    $last_message = "";
                    $mid_res = mysqli_query($conn, "SELECT message, users_id FROM messages WHERE ( (users_id='$users_id' AND receiver_id='{$row['id']}') OR (users_id='{$row['id']}' AND receiver_id='$users_id')) AND is_delete='0' ORDER BY id DESC LIMIT 1");
                    if(mysqli_num_rows($mid_res)>0)
                    {
                        $mrow = mysqli_fetch_assoc($mid_res);
                    ...
                    ?>
                        <form>
                            ...
                            <div class="user-info">
                                ...
                                <small class="last-message"><?php echo htmlspecialchars($last_message); ?></small>
                            </div>
                        </button>
                    </form>
                    <?php
